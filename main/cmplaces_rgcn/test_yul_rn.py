...
def test_ckpt(ckpt_path, m1_q_indices, m1_r_indices, m2_q_indices, m2_r_indices):
    ...
    with tf.Session(config=config) as sess:
        ...
        m2_t = time.time()
        cost = trans_time(m2_t-m1_t)
        print('----------   Img query retrieval done, Avg map : %6.4f, Cost time: %02d:%02d:%02d   ----------' % (mAP2,cost[0],cost[1],cost[2]))
        m3_t = time.time()
        ...
        m4_t = time.time()
        cost_ = trans_time(m4_t-m3_t)
        print('----------   text query retrieval done, Avg map : %6.4f, Cost time: %02d:%02d:%02d   ----------' % (mAP1,cost_[0],cost_[1],cost_[2]))
...
if __name__ == '__main__':
    ...
    for ckpt_name in ckpt_names:
        ...
        print("Model name: %10s Txt: %6.4f, Img: %6.4f, Avg:%6.4f " % (ckpt_name, mAP1, mAP2, (mAP1 + mAP2) / 2))
    ...
    print("Model name: %2s Txt: %6.4f, Img: %6.4f, Avg:%6.4f " %
